/*global addEvent, Calendar, cancelEventPropagation, findPosX, findPosY,
 * getStyle, get_format, gettext, interpolate, ngettext, quickElement,
 * removeEvent*/
...
var DateTimeShortcuts = {
  ...
  timezoneWarningClass :
      'timezonewarning', // class of the warning for timezone mismatch
  timezoneOffset : 0,
  init : function() {
  ...
  addTimezoneWarning : function(inp) {
    ...
    var message;
    if (timezoneOffset > 0) {
      message = ngettext('Note: You are %s hour ahead of server time.',
                         'Note: You are %s hours ahead of server time.',
                         timezoneOffset);
    } else {
      timezoneOffset *= -1;
      message = ngettext('Note: You are %s hour behind server time.',
    ...
    $(inp).parent().append($('<br>')).append($warning);
  },
  ...
  addClock : function(inp) {
    ...
    quickElement('span', clock_link, '', 'class', 'clock-icon', 'title',
    ...
    var time_link = quickElement("a", quickElement("li", time_list),
                                 gettext("Now"), "href", "#");
    addEvent(time_link, 'click', function(e) {
      e.preventDefault();
      DateTimeShortcuts.handleClockQuicklink(num, -1);
    });
    time_link = quickElement("a", quickElement("li", time_list),
  ...
  handleClockQuicklink : function(num, val) {
    ...
    DateTimeShortcuts.clockInputs[num].value =
  ...
  addCalendar : function(inp) {
    ...
    quickElement('span', cal_link, '', 'class', 'date-icon', 'title',
    ...
    //          <a href="#">Yesterday</a> | <a href="#">Today</a> | <a
    //          href="#">Tomorrow</a>
    //     </div>
    //     <p class="calendar-cancel"><a href="#">Cancel</a></p>
    // </div>
    ...
    DateTimeShortcuts.calendars[num] =
        new Calendar(DateTimeShortcuts.calendarDivName2 + num,
    ...
    var day_link =
        quickElement('a', shortcuts, gettext('Yesterday'), 'href', '#');
  ...
  drawPrev : function(
      num) { DateTimeShortcuts.calendars[num].drawPreviousMonth(); },
  drawNext : function(
      num) { DateTimeShortcuts.calendars[num].drawNextMonth(); },
  handleCalendarCallback : function(num) {
    ...
    return function(y, m, d) {
      DateTimeShortcuts.calendarInputs[num].value =
          new Date(y, m - 1, d).strftime(format);
